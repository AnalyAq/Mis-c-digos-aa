...
path <- "C:/Users/USER/OneDrive/UNALM/MATERIAL/6_CICLO/Técnicas Estadísticas/costa_sur/homogenizacion/prueba/"
setwd(path)
...
# poner los datos en formato para climatol
...
#aca se puede poner mindat si hay pocos datos
daily2climatol(stfile = "Est.csv",stcol=1:6,datcol = c(1:3,ncol),varcli= varcli,anyi= inic, anyf=fin,dec=".",header=T,na.strings = -99.9)

#homogen("tmin",anyi= 1970, anyf=2019,onlyQC = T)
...
dd2m(varcli,anyi= inic, anyf=fin,valm= 2, namax=35)
varcli
...
homogen(paste0(varcli,"-m"),anyi= inic, anyf=fin,std=3)
paste0(varcli,"-m")
...
homogen(varcli,anyi= inic, anyf=fin,metad=T)
varcli
#Cargando resultados
load(paste0(varcli,"_",inic,"-",fin,".rda"))   
paste0(varcli,"_",inic,"-",fin,".rda")
...
dahstat(varcli,anyi= inic, anyf= fin,stat="series")
paste0(varcli,"_",inic,"-",fin,".series")

#Importando serie
homogen <- read.table(paste0(varcli,"_",inic,"-",fin,"_series.csv"),sep=",",header = T)
paste0(varcli,"_",inic,"-",fin,"_series.csv")
homogen
...
homogen_sel <- data.frame(Fecha=homogen$Date,ho00000830=homogen$ho00000830,ho00000833=homogen$ho00000833,
                          ho00000840=NA,ho00000846=homogen$ho00000846,
                          ho00157102=homogen$ho00157102)

write.table(homogen_sel,file ="tmin_diario_complete.csv",sep = ",",row.names=F)
...
write.table(smy,"tmin_complete_estadisticas.csv",row.names = T)
...
ggsave(file="tmin_complete.png", width = 15, height = 9)
